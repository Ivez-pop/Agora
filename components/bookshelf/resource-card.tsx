...
export default function ResourceCard({ resource }: ResourceCardProps) {
  return (
    <article className={resource.imageUrl ? "resource-card has-resource-image" : "resource-card"}>
      {resource.imageUrl ? (
        <a className="resource-image-link" href={resource.resourceLink} target="_blank" rel="noreferrer">
      ...
      <div className="resource-card-content">
        ...
        <h2>
          <a href={resource.resourceLink} target="_blank" rel="noreferrer">{resource.title}</a>
        </h2>
        {resource.author ? <p className="resource-author">by {resource.author}</p> : null}
      </div>
      <div className="resource-card-actions">
        <a className="secondary-button" href={resource.resourceLink} target="_blank" rel="noreferrer">
          Open Resource
        </a>
      </div>
    </article>
